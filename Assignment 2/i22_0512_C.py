...
for i in range(len(inp)):
    for j in range(len(inp[0])):
        if (inp[i][j][0] != 47) and (inp[i][j][1] != 171 ) and (inp[i][j][2] != 39):          # if the rgb value is that of green it does not pick the rgb value
            res_img[i][j] = inp[i][j]                  #copies the data of rgb values of non-green ppixel to target background screen 
...
# **Q2:** Do the following operation (given below) on the numpy list
...
# ##### you can select column in Numpy by arr[ :,2].. It will select 2nd column
...
max_values_in_col = np.zeros(6)
for i in range(6):
    temp = np.nanmax(data[:,i])              #using functions to find max value in each column 
    max_values_in_col[i] = temp              # stores the max values in a np array 
...
temp = 0
min_values_in_col = np.zeros(6)
for i in range(6):
    temp = np.nanmin(data[:,i])            # similar as above code but for min value
    min_values_in_col[i] = temp
...
maxval = np.nanmax(data)                    ## checking max value in data
maxval_index = np.where(data == maxval)         ## using function to find what is the index of the max value by comparing
print("Index where the max value is " ,maxval_index)

minval = np.nanmin(data)                           ## similar as above but for minimum value
min_val_index = np.where(data == minval)
print("Index where the min value is " ,min_val_index)
...
Average = np.zeros(6)
Std_dev = np.zeros(6)
Variance = np.zeros(6)                ## NumPy arrays for averages std deviation and variance
for i in range(6):
    temp = np.nanmean(data[:,i])
    Average[i] = temp
    temp = np.nanstd(data[:,i],axis = 0)
    Std_dev[i] = temp
    temp = np.nanvar(data[:,i])             ## calculating the average,std and variance using the nanmean, nanstd and nanvar func
    Variance[i] = temp
print(f"The averages of columns are {Average} ")
print(f"The Standard Deviation of columns are {Std_dev} ")
print(f"The Variances of columns are {Variance} ")
...
col_sum = np.zeros(6)
for i in range(6):
    col_sum[i] = np.nansum(data[:,i])           ## using function to sum each column
print(f"Sum of each column is {col_sum}" )
...
for i in range(1):
    np.nan_to_num(data[:,i],False ,nan = max_values_in_col[i])   ## using nan_to_num function to fill nan with max values of each coumn using loop
data[:,0]
...
np.nan_to_num(data[:,1],False ,nan = np.nanmin(data[:,1]))     ## same as above code
#for i in range(400):
#    for j in range(2):
#        print(data[i][j])
data[:,1]
...
np.nan_to_num(data[:,2],False ,nan = np.nanmean(data[:,2]))     ## same as above but nan filled with mean of column
...
for i in range(len(data)):
    for j in range(len(data[0])-2):             ## restricting it so it does not go to column 5 and 6 
        if np.isnan(data[i][j]):
            data[i][j] = data[i+1][j]           ## filling nan wiht next value
data[:,3]
...
for i in range(len(data)):
    for j in range(len(data[0])-1):      ## restricting it to go to last column
        if np.isnan(data[i][j]):
            data[i][j] = data[i-1][j]      ## filling nan with value above it
data[:,4]
...
removing_data = np.array([])
for i in range(len(data)):
    nan_counter = 0
    for j in range(len(data[0])):             
        if (np.isnan(data[i][j])) and j == 5:     ## checking in last column if value is nan then it will not be copied to new np array
            nan_counter+=1
    if nan_counter == 0:
        removing_data = np.append(removing_data, data[i])  ##  copying the previous arrays and missing the rows where nan is found

removing_data = np.reshape(removing_data, (-1,6))    ## reshaping the array back to original shape
np.shape(removing_data)
removing_data
...
## this standardization has been performed on the array that has all the above things applied meaning nan values removed

Aver = np.zeros(6)
Std_d = np.zeros(6)
Vari = np.zeros(6)
for i in range(6):
    temp = np.nanmean(removing_data[:,i])
    Aver[i] = temp
    temp = np.nanstd(removing_data[:,i],axis = 0)
    Std_d[i] = temp
    temp = np.nanvar(removing_data[:,i])
    Vari[i] = temp

new_dataset = np.array([])


for i in range(len(removing_data)):
    central_val = 0
    scaled_val = 0
    temp = np.array([])
    for j in range(len(removing_data[0])):
        central_val = removing_data[i][j] - Aver[j]            ## subtrating the average for obtaing central value
        scaled_val = central_val/Std_d[j]                       ## for getting standard value
        temp = np.append(temp, scaled_val)
    new_dataset = np.append(new_dataset, temp)
    
new_dataset= np.reshape(new_dataset,(-1,6))                  ##  reshaping the new np array formed to the shape of original array
...
data = data.flatten()         ## using function to make array 1-D
data
...
np.reshape(data,(400,6))            ## reshaping the array back to original state
...
# **Q3:** In a post-apocalyptic world where Earth has become uninhabitable due to a nuclear fallout, a group of survivors is looking to you as their last hope. You are a skilled navigator on a mission to find the nearest potentially habitable planet to ensure the survival of humanity.
...
dist_from_planets = np.zeros(6)
len(planet_coordinates)
for i in range(len(planet_coordinates)):
    temp1 = np.zeros(3)
    t1 = 0
    for j in range(len(spacecraft_position)):
        temp = (planet_coordinates[i][j] - spacecraft_position[j]) **2
        temp1[j] = temp
        SUM = sum(temp1) 
        t1 = SUM **(1/2)
    dist_from_planets[i] = t1              ## finding the distance of spacecraft from each planet 


# In[35]:


Min_dist = np.min(dist_from_planets)
arg_ind = 0 
for i in range(len(planet_names)):          ## comparing the min distance with the planet so to find its name
    if dist_from_planets[i] == Min_dist:
        print(f"The Planet from which the distance is minimum is {planet_names[i]} with distance of {Min_dist}")
        arg_ind = i
        break
...
origin_planet = planet_coordinates[arg_ind]
angles = np.zeros(6)
scalar_prod = np.zeros(6)
mag_arr = np.zeros(6)
for i in range(len(planet_coordinates)):
    temp2 = np.zeros(len(origin_planet))
    t_for_planet = 0
    for j in range(len(origin_planet)):
        temp = planet_coordinates[i][j]**2
        temp2[j] = temp
        SUM = sum(temp2)
        t_for_planet = SUM**(1/2)
    mag_arr[i] = t_for_planet          ## magnitude of every other planet

magnitude_of_origin = (origin_planet[0]**2 + origin_planet[1]**2 + origin_planet[2]**2)**(1/2)  ## magnitude of kepler 442 b

## for finding angle using arc cos (u.v/|u||v|) formula 

for i in range(len(planet_coordinates)):
    t1 = 0
    temp_for_sum = np.zeros(len(origin_planet))
    for j in range(len(origin_planet)):
        t1 = origin_planet[j] * planet_coordinates[i][j]
        temp_for_sum[j] = t1
    Summ = sum(temp_for_sum)
    scalar_prod[i] = Summ                       ## scalar product of origin planet and other planets
    

for i in range(len(mag_arr)):
    angles[i] = np.arccos(scalar_prod[i]/(magnitude_of_origin * mag_arr[i]))    ## using formula

for i in range(len(planet_names)):
    print(f"The angle  between {planet_names[i]} and {planet_names[1]} is {angles[i]} ")
